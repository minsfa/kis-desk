...
데이터 소스(전부 읽기전용):
  ...
  - 금현물 시세(금 99.99_1kg 종가, 원/g): KRX OPEN API gold_bydd_trd (tools/krx_api.py)
...
COLUMNS = [
    ...
    # 4단계: KRX 금현물 시세 (KRX 금시장 API, 끝에 append)
    #   gold_krw_g = 금 99.99_1kg(ISU_CD=04020000) 종가, 원/g
    "gold_krw_g",
]
...
def _collect(days: int) -> tuple[dict[str, dict], list[str]]:
    ...
    if not key:
        ...
        # 4단계: KRX 금현물 시세 — 금 99.99_1kg 종가(원/g).
        #   금시장 일별 전종목을 날짜별로 받음. 빈 배열(주말/휴일/당일/미갱신)이나
        #   해당 종목 없으면 None → 공란 유지(graceful).
        gold_fail = 0
        for dt in sorted(rows_by_date):
            try:
                gold = krx_api.fetch_gold(dt, key=key)
            except Exception as e:
                gold_fail += 1
                warns.append(f"금현물 {dt} 실패 — gold_krw_g 공란: {str(e)[:60]}")
                continue
            if gold is None:  # 데이터 없음 → 공란 유지
                continue
            _merge_stage2(rows_by_date, dt, gold_krw_g=gold)
        if gold_fail:
            warns.append(f"금현물 실패 {gold_fail}건 — 해당 날짜만 공란.")
...
def cmd_show(n: int):
    ...
    # ── 4단계: KRX 금현물 시세 ──────────────────────────────────────────
    print("\n[4단계 / KRX 금현물] 금 99.99_1kg(ISU_CD=04020000) 종가 = 원/g")
    h4 = f"{'일자':>11} {'금현물종가':>14}"
    print(h4)
    print("─" * len(h4))
    for r in recent:
        print(f"{_fmt_date(r['date']):>11} "
              f"{_gold_s(r.get('gold_krw_g')):>14}")
